...
def compute_z(
    ...
    def edit_output_fn(cur_out, cur_layer):
        ...
        if cur_layer == hparams.layer_module_tmp.format(layer):
            ...
            # Handle dims: (batch, seq, hidden) or (seq, batch, hidden) or (seq, hidden)
            if layer_out.dim() == 3:
                batch_first = layer_out.size(0) == 1  # batch size is 1
                if target_init is None:
                    if batch_first:
                        target_init = layer_out[0, lookup_idxs[0], :].detach().clone()
                    else:
                        target_init = layer_out[lookup_idxs[0], 0, :].detach().clone()
                # apply delta at lookup positions
    ...
    for it in range(hparams.v_num_grad_steps):
        ...
        # Ensure full_repr shape is (batch, seq, hidden)
        if layer_out.dim() == 3:
            full_repr = layer_out
